...
export const files = pgTable("files", {
  ...
  thumbnailUrl: text("thumbnail_url"), // optiional thumbanial for docs or images...
  
  // Ownership and hierarchy
  userId: text("user_id").notNull(),
  parentId: uuid("parent_id"), // it is a parent folder id, it will remain null for root items

  // File/folder flags : clicked marked as star
  isFolder: boolean("is_folder").default(false).notNull(),
  isStarred: boolean("is_starred").default(false).notNull(),
  isTrash: boolean("is_trash").default(false).notNull(),

  // Timestamps at which things are created at:
 ...
 */
export const filesRelations = relations(files, ({ one, many }) => ({ // 1 to many
